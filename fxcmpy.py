...
class fxcmpy(object):
    ...
    def __init__(self, user='', password='', config_file='', log_file='', log_level=''):
        self.logger = None
        self.config_file = ''
        ...
        if user != '' and  password != '':
            ...
            if os.path.isfile(config_file):
                ...
                raise IOError('Can not find configiguration file: %s' %config_file)
            try:
                self.user = self.__get_config_value__('FXCM', 'user')
            except:
                raise ValueError('Can not find user in configuration file %s' % self.config_file)
            try:
                self.password = self.__get_config_value__('FXCM', 'password')
            except:
                raise ValueError('Can not find %s password in configuration file %s' % (self.user, self.config_file))
        else:
        ...
        if log_level != '':
            if log_level in LOG_LEVELS:
                ...
                raise ValueError('log_level must be one of %s' %LOG_LEVELS.keys()) 
        elif self.config_file != '':
            try:
                log_level =  LOG_LEVELS[self.__get_config_value__('FXCM', 'log_level')]
            except:
    ...
    def is_connected(self):
        """ Returns True if the socket connection is established and False else """
    ...
    def get_model(self, models=list()):
        ...
        data = self.__handle_request__(method='trading/get_model', params={'models':models})    
    ...
    def subscribe_market_data(self, symbol='', add_callbacks=()):
        ...
        for func in add_callbacks:
            if not callable(func):
                ...
                raise ValueError('The content of add_callbacks must be callable')
            else:
                if symbol not in self.add_callbacks:
                    self.add_callbacks[symbol] = dict()
                self.logger.info('Adding callback method %s for symbol %s' %(func.__name__, symbol))
    ...
    def subscribe_data_model(self, model='', add_callbacks=()):
        ...
        for func in add_callbacks:
            if not callable(func):
                ...
                self.logger.info('Adding callback method %s for model %s' %(func.__name__, model))
                self.add_callbacks[model][func.__name__] = func
                
        params = {'models': model}
        data = self.__handle_request__(method='trading/subscribe', params=params)    
        print(data) 
    ...
    def unsubscribe_data_model(self, model=''):
        ...
        params = {'models': model}
        data = self.__handle_request__(method='trading/unsubscribe', params=params)    
    ...
    def open_trade(self, account_id, symbol, side, 
                   amount, time_in_force, rate=0, stop=None, trailing_step=None):
        params = {
        ...
        data = self.__handle_request__(method='trading/open_trade', params=params)    
        print(data)
    
    def change_order(self, order_id, amount, rate=0, order_range=0, trailing_step=None):
        params = {
                  'order_id': order_id,
                  'rate': rate,
                  'range': order_range,
        ...
        data = self.__handle_request__(method='trading/change_order', params=params)
        print(data)
    ...
    def delete_order(self, order_id):
        ...
        data = self.__handle_request__(method='trading/delete_order', params=params)
        print(data)
    ...
    def create_entry_order(self, account_id, symbol, side, amount, limit, 
        ...
        data = self.__handle_request__(method='trading/create_entry_order', params=params)
        print(data)

    def create_oco_order(self, account_id, symbol, side, side2,  amount, is_in_pips, 
                         time_in_force, at_market, order_type, expiration,
                         limit, limit2, rate, rate2,  stop, stop2,  trailing_step,
                         trailing_step2, trailing_stop_step, trailing_stop_step2 ):
        ...
        data = self.__handle_request__(method='trading/simple_oco', params=params)
        print(data)
    ...
    def get_candles(self, offer_id, period, number='', start=None, stop=None):
        ...
        if period not in self.PERIODS:
            self.logger.error('Error in get_candles: Illegal period: %s' %period)
            raise ValueError('period must be one of %s' %self.PERIODS)
        if number != '' and (type(number) != int or number > 10000):
            self.logger.error('Error in get_candles: Illegal prameter number: %s' %number)
            raise ValueError('number must be a integer smaller or equal to 10000')
        ...
        if start:
            if isinstance(start, dt.datetime) or isinstance(start, dt.date):
                start = (start - dt.datetime(1970, 1, 1)) / dt.timedelta(seconds=1)
            try:
                start = int(start)
            except:
                self.logger.error('Error in get_candles: Illegal value for start: %s' % start)
                raise ValueError('start must be an integer')
            params['From'] = start

        if stop:
            if isinstance(stop, dt.datetime) or isinstance(stop, dt.date):
                stop = (stop - dt.datetime(1970, 1, 1)) / dt.timedelta(seconds=1)
            try:
                stop = int(stop)
            except:
                self.logger.error('Error in get_candles: Illegal value for stop: %s' % stop)
                raise ValueError('stop must be an integer')
            params['To'] = stop

        data = self.__handle_request__(method='candles/%s/%s' % (offer_id, period), params=params)
    ...
    def __connect__(self):
        try:
            self.socket = SocketIO(self.trading_url, self.port, params={'access_token': self.access_token})
    ...
    def __request_access_token__(self):
        ...
        if req.status_code != 200:
            self.logger.error('FXCM reject request for temporay access token with status %s and message %s'
        ...
        try:
            ...
            self.logger.error('Can not parse server answer to json object: %s' %req.text)
        ...
        req = requests.get(self.trading_url+'/authenticate', params=auth2_params)
        if req.status_code != 200:
            self.loggin.error('FXCM reject request for temporay access token with status %s and message %s' 
			 % (req.status_code, req.text))
            raise ServerError('Request returns status code %s' %req.status_code)
        ...
        if not data['response']['executed']:
            if 'error' in data['response'] and data['response']['error'] != '':
                self.logger.error('Server reports an error: %s' % data['response']['error'])
                raise ServerError('FXCM Server reports an error: %s' % data['response']['error'])
            else:
                self.logger.error('FXCM Server reports an unknown error: %s' % data['response'])
                raise ServerError('FXCM Server returns an unknown error')
                
        if 'access_token' not in data:
            self.logger.error('Can not find access token in server answer: %s' % data)
    ...
    def __handle_request__(self, method="", params={}):
        ...
        if type(params) is not dict:
            self.logger.debug('Error in __handle__requests__: params must be of type dict')
        ...
        self.logger.debug('Sending request to %s/%s, paramerter: %s' %( self.trading_url, method, params))
        req = requests.get('%s/%s' %( self.trading_url, method), params=params)
        if req.status_code != 200:
            self.logger.error('FXCM reject request %s with status %s and message %s'
                              % (method, req.status_code, req.text))

            raise ServerError('Request returns status code %s and message "%s"' 
                               % (req.status_code, urllib.parse.unquote(req.text)))
    ...
    def __get_config_value__(self, section, key):
        ...
        if key not in config[section]:
            ...
            raise ValueError('Can not find key %s in section %s of %s'
                         %(key, section, self.config_file))
        if self.logger:
            self.logger.debug('Found config value %s for key %s in section %s' %(config[section][key], key, section))
